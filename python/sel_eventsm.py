...
class Cutflow():
    def __init__(self):
        ...
        self.h_evtflw.GetXaxis().SetBinLabel(2,'visible energy <108')
        self.h_evtflw.GetXaxis().SetBinLabel(3,'N photon <= 1')
        self.h_evtflw.GetXaxis().SetBinLabel(4,'Pt_dilepton<55')
        self.h_evtflw.GetXaxis().SetBinLabel(5,'|Pz|<50')
        self.h_evtflw.GetXaxis().SetBinLabel(6,'88<Mmumu<94')
        self.h_evtflw.GetXaxis().SetBinLabel(7,'123<Mrecoil')
        self.h_evtflw.GetXaxis().SetBinLabel(8,'m_e_photon<2.4')
        ...
        self.h_before_cut_ep = ROOT.TH1F('before_cut_ep','before_cut_ep',1000,0,5)
        ...
        self.h_after_cut_Mmumu= ROOT.TH1F('after_cut_Mmumu','after_cut_M_{#mu^{+}#mu^{-}}',120,40,100)
        self.h_after_cut_Mrecoil = ROOT.TH1F('after_cut_Mrecoil','after_cut_M_{Recoil}',140,100,170)
        self.h_after_cut_ep = ROOT.TH1F('after_cut_ep','after_cut_ep',1000,0,5)
...
##        m_e_other=t_in.m_energy_visible-abs(t_in.m_p_leptonm[3])-abs(t_in.m_p_leptonp[3])
##        if not (abs(m_e_other)<1000):
##            return False
...
#        if not (t_in.m_energy_visible>98 and t_in.m_energy_visible<108 ):
#            return False
#        self.N[5]+=1
#        self.h_evtflw.Fill(5) 
##        if not (abs(t_in.m_sum_p_photon[3])<6):
##            return False
##        self.N[6]+=1
##        self.h_evtflw.Fill(6) 	
#        if not (t_in.m_m_dimu<110 and t_in.m_m_dimu>60):
#            return False
##        if not (t_in.m_mine_lepton>25 and t_in.m_maxe_lepton<80 ):
##            return False		
#        self.N[6]+=1
#        self.h_evtflw.Fill(6)     
        m_pp_lepton=math.sqrt(t_in.m_p_dilepton[0]*t_in.m_p_dilepton[0]+t_in.m_p_dilepton[1]*t_in.m_p_dilepton[1]+t_in.m_p_dilepton[2]*t_in.m_p_dilepton[2])
...
#        if not (costheta_leptonm<0.6 and costheta_leptonp>-0.6 ):
#            return False    
#        a = (t_in.m_e_leptonm+t_in.m_e_leptonp)/t_in.m_energy_visible
#        if not (t_in.TauTauImpact<0.0011):
#            return False
#        self.N[7]+=1
...
#        self.plot_after_cut(t_in,m_l_ep)
    ...
    def fill_root(self,t_in,t_out,m_l_ep):
        ...
        if abs(t_in.m_sum_p_photon[3])>1000:
            t_in.m_sum_p_photon[3]=0
        ...
        self.m_theta_dilepton[0]=t_in.m_theta_dilepton
        ...
        self.m_e_ep[0]=(t_in.m_p_leptonm[3]+t_in.m_p_leptonp[3])/t_in.m_energy_visible

...
void fitbkg() 
{
   ...
   TString rootfile;
   rootfile = "./run/total/hist/total_bkg.root";
    
   TString  epsname;  
   epsname="./fig/fithiggb.pdf";
...
//
  ...
  // RooHistPdf bkgh("bkg1","bkgshape", y, data2, 0);
   ...
   RooRealVar mean("mean_CB","mean_CB",125.3,125,126);
   RooRealVar sigma("sgm_CB","sgm_CB",0.62,0.30,0.80);
   RooRealVar alpha("alpha_CB","alpha_CB",-1.1,-10.,5.);
   RooRealVar n("n","n",1,0.80,2);
   ...
   //Argus background
   RooRealVar fira1("fira1", "fira1",0.4);
   RooRealVar fira2("fira", "fira2",2.5,-999,999);
   RooRealVar argpar("argpar","argus shape parameter",-0.8) ;
...
// RooRealVar co4("co4","coefficienct #4",0,-999., 999.,-10,10);
  ...
  // RooRealVar nbkg("nbkg", "background number",0, mbkg);
   ...
   //sum.plotOn(xframe,Components(argus),LineStyle(2), LineColor(3));
   sum.plotOn(xframe,Components(bkg),LineStyle(2), LineColor(2));
   //gPad->SetLogy();

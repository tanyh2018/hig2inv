// -*- C++ -*- //  
///cvmfs/cepc.ihep.ac.cn/software/cepcsoft/x86_64-sl6-gcc49/cepcsoft/0.1.0-rc9/Framework/LCIO/02-04-03/include
    ...
    if(evt){
        try{
            ...
            if(buildZToff()){
                ...
                //selectIsoLepton(nLepton, Col_Leps, P4_Jet);
...
void hig2inv::book_histogram() {
    ...
    h_evtflw->GetXaxis()->SetBinLabel(2, "Lepton Number");
    h_evtflw->GetXaxis()->SetBinLabel(3, "Recoil Mass");
    h_evtflw->GetXaxis()->SetBinLabel(4, "");
...
void hig2inv::book_tree() { 
    ...
    m_tree->Branch("m_Neutral_PID",  &m_Neutral_PID);
    m_tree->Branch("m_charged_PID",  &m_charged_PID);
...
//new qq channel branch
    ...
    m_tree->Branch("m_e_charged", &m_e_charged);
    ...
    m_tree->Branch("m_mc_maxpe_electron",  &m_mc_maxpe_electron,  "m_mc_maxpe_electron/F");
    ...
    m_tree->Branch("m_visible_p",  &m_visible_p,  "m_visible_p/F");
    m_tree->Branch("m_visible_pt",  &m_visible_pt,  "m_visible_pt/F");
    m_tree->Branch("m_n_IsoMuonP",  &m_n_IsoMuonP,  "m_n_IsoMuonP/I");
    m_tree->Branch("m_n_IsoMuonM",  &m_n_IsoMuonM,  "m_n_IsoMuonM/I");
    m_tree->Branch("m_n_IsoMuon",  &m_n_IsoMuon,  "m_n_IsoMuon/I");
    m_tree->Branch("m_n_IsoEletronP",  &m_n_IsoEletronP,  "m_n_IsoEletronP/I");
    m_tree->Branch("m_n_IsoEletronM",  &m_n_IsoEletronM,  "m_n_IsoEletronM/I");
    m_tree->Branch("m_n_IsoEletron",  &m_n_IsoEletron,  "m_n_IsoEletron/I");

    m_tree->Branch("m_m_Isdimu",  &m_m_Isdimu,  "m_m_Isdimu/F");
    m_tree->Branch("m_e_Isdimu",  &m_e_Isdimu,  "m_e_Isdimu/F");
    m_tree->Branch("m_p_Isdimu",  &m_p_Isdimu,  "m_p_Isdimu/F");
    m_tree->Branch("m_e_Isdimurec",  &m_e_Isdimurec,  "m_e_Isdimurec/F");
    m_tree->Branch("m_p_Isdimurec",  &m_p_Isdimurec,  "m_p_Isdimurec/F");
    m_tree->Branch("m_m_Isdimurec",  &m_m_Isdimurec,  "m_m_Isdimurec/F");
    m_tree->Branch("m_m_Isdie",  &m_m_Isdie,  "m_m_Isdie/F");
    m_tree->Branch("m_e_Isdie",  &m_e_Isdie,  "m_e_Isdie/F");
    m_tree->Branch("m_p_Isdie",  &m_p_Isdie,  "m_p_Isdie/F");
    m_tree->Branch("m_e_Isdierec",  &m_e_Isdierec,  "m_e_Isdierec/F");
    m_tree->Branch("m_p_Isdierec",  &m_p_Isdierec,  "m_p_Isdierec/F");
    m_tree->Branch("m_m_Isdierec",  &m_m_Isdierec,  "m_m_Isdierec/F");
    m_tree->Branch("m_muz_theta",  &m_muz_theta,  "m_muz_theta/F");
    m_tree->Branch("n_muon_Ptrack", &n_muon_Ptrack, "n_muon_Ptrack/F");
    m_tree->Branch("n_muon_Mtrack", &n_muon_Mtrack, "n_muon_Mtrack/F");
    m_tree->Branch("n_muon_track", &n_muon_track, "n_muon_track/F");
    // Jets
    ...
    m_tree->Branch("mc_n_charge", &mc_n_charge, "mc_n_charge/I");
    ...
    m_tree->Branch("mc_init_leptonp_e",  &m_mc_init_leptonp_e,   "mc_init_leptonp_e/F");
    m_tree->Branch("mc_init_leptonp_p",  &m_mc_init_leptonp_p,   "mc_init_leptonp_p/F");
    m_tree->Branch("mc_init_leptonp_pt", &m_mc_init_leptonp_pt,  "mc_init_leptonp_pt/F");
    m_tree->Branch("mc_init_leptonp_pz", &m_mc_init_leptonp_pz,  "mc_init_leptonp_pz/F");

    m_tree->Branch("mc_init_leptonp_phi", &m_mc_init_leptonp_phi,  "mc_init_leptonp_phi/F");
    m_tree->Branch("mc_init_leptonp_theta", &m_mc_init_leptonp_theta,  "mc_init_leptonp_theta/F");
    
    m_tree->Branch("mc_init_leptonm_e",  &m_mc_init_leptonm_e,   "mc_init_leptonm_e/F");
    m_tree->Branch("mc_init_leptonm_p",  &m_mc_init_leptonm_p,   "mc_init_leptonm_p/F");
    m_tree->Branch("mc_init_leptonm_pt", &m_mc_init_leptonm_pt,  "mc_init_leptonm_pt/F");
    m_tree->Branch("mc_init_leptonm_pz", &m_mc_init_leptonm_pz,  "mc_init_leptonm_pz/F");

    m_tree->Branch("mc_init_leptonm_phi", &m_mc_init_leptonm_phi,  "mc_init_leptonm_phi/F");
    m_tree->Branch("mc_init_leptonm_theta", &m_mc_init_leptonm_theta,  "mc_init_leptonm_theta/F");
    
    m_tree->Branch("mc_init_dilepton_m",  &m_mc_init_dilepton_m,   "mc_init_dilepton_m/F");
    m_tree->Branch("mc_init_dilepton_e",  &m_mc_init_dilepton_e,   "mc_init_dilepton_e/F");
    m_tree->Branch("mc_init_dilepton_p",  &m_mc_init_dilepton_p,   "mc_init_dilepton_p/F");
    m_tree->Branch("mc_init_dilepton_pt", &m_mc_init_dilepton_pt,  "mc_init_dilepton_pt/F");
    m_tree->Branch("mc_init_dilepton_pz", &m_mc_init_dilepton_pz,  "mc_init_dilepton_pz/F");
    m_tree->Branch("mc_init_dilepton_rec_m", &m_mc_init_dilepton_rec_m,  "mc_init_dilepton_rec_m/F");
    m_tree->Branch("mc_init_dilepton_dphi", &m_mc_init_dilepton_dphi,  "mc_init_dilepton_dphi/F");
    m_tree->Branch("mc_init_dilepton_dang", &m_mc_init_dilepton_dang,  "mc_init_dilepton_dang/F");

    m_tree->Branch("m_mc_p_dilepton", &m_mc_p_dilepton, "m_mc_p_dilepton[4]/F");
    m_tree->Branch("m_mc_p_redilepton", &m_mc_p_redilepton, "m_mc_p_redilepton[4]/F");
    ...
    m_tree->Branch("m_sum_e_charged", &m_sum_e_charged,  "m_sum_e_charged/F");
    m_tree->Branch("m_sum_e_photon", &m_sum_e_photon,  "m_sum_e_photon/F");
    m_tree->Branch("m_sum_e_501", &m_sum_e_501,  "m_sum_e_501/F");
    m_tree->Branch("m_sum_e_21120", &m_sum_e_21120,  "m_sum_e_21120/F");
    m_tree->Branch("m_sum_e_neutral", &m_sum_e_neutral,  "m_sum_e_neutral/F");
    m_tree->Branch("m_sum_muon_e", &m_sum_muon_e,  "m_sum_muon_e/F");
    m_tree->Branch("m_sum_electron_e", &m_sum_electron_e,  "m_sum_electrone/F");
    m_tree->Branch("m_total_clu_energy", &m_total_clu_energy,  "m_total_clu_energy/F");


    m_tree->Branch("mc_init_qq_electron_e",  &m_mc_init_qq_electron_e);
    m_tree->Branch("mc_init_qq_muon_e",  &m_mc_init_qq_muon_e);
    m_tree->Branch("mc_init_qq_photon_e",  &m_mc_init_qq_photon_e);
    m_tree->Branch("mc_init_qq_neutral_e",  &m_mc_init_qq_neutral_e);    
    m_tree->Branch("mc_init_qq_charge_e",  &m_mc_init_qq_charge_e);    

    m_tree->Branch("m_mc_nparents", &m_mc_nparents);
    m_tree->Branch("m_mc_ndaughters", &m_mc_ndaughters);
    m_tree->Branch("m_mc_charge", &m_mc_charge);

    m_tree->Branch("mc_n_qq_neutral", &mc_n_qq_neutral,  "mc_n_qq_neutral/I");
    m_tree->Branch("mc_n_qq_photon", &mc_n_qq_photon,  "mc_n_qq_photon/I");
    m_tree->Branch("mc_n_qq_muon", &mc_n_qq_muon,  "mc_n_qq_muon/I");
    m_tree->Branch("mc_n_qq_electron", &mc_n_qq_electron,  "mc_n_qq_electron/I");
    m_tree->Branch("mc_n_qq_charge", &mc_n_qq_charge,  "mc_n_qq_charge/I");

    m_tree->Branch("m_mc_m_visible", &m_mc_m_visible,  "m_mc_m_visible/F");
    m_tree->Branch("m_mc_energy_visible", &m_mc_energy_visible,  "m_mc_energy_visible/F");
    m_tree->Branch("m_mc_visible_pt", &m_mc_visible_pt,  "m_mc_visible_pt/F");
    m_tree->Branch("m_mc_visible_p", &m_mc_visible_p,  "m_mc_visible_p/F");
    m_tree->Branch("m_mc_miss_e", &m_mc_miss_e,  "m_mc_miss_e/F");

    m_tree->Branch("m_mc_miss_m", &m_mc_miss_m,  "m_mc_miss_m/F");
    m_tree->Branch("m_mc_sum_muon_e", &m_mc_sum_muon_e,  "m_mc_sum_muon_e/F");
    m_tree->Branch("m_mc_sum_muon_m", &m_mc_sum_muon_m,  "m_mc_sum_muon_m/F");
    m_tree->Branch("m_mc_sum_electron_e", &m_mc_sum_electron_e,  "m_mc_sum_electron_e/F");
    m_tree->Branch("m_mc_sum_electron_m", &m_mc_sum_electron_m,  "m_mc_sum_electron_m/F");
    m_tree->Branch("m_mc_sum_photon_e", &m_mc_sum_photon_e,  "m_mc_sum_photon_e/F");
    m_tree->Branch("m_mc_sum_photon_m", &m_mc_sum_photon_m,  "m_mc_sum_photon_m/F");

    m_tree->Branch("m_mc_sum_charge_e", &m_mc_sum_charge_e,  "m_mc_sum_charge_e/F");
    m_tree->Branch("m_mc_sum_charge_m", &m_mc_sum_charge_m,  "m_mc_sum_charge_m/F");
    m_tree->Branch("m_mc_sum_neutral_e", &m_mc_sum_neutral_e,  "m_mc_sum_neutral_e/F");
    m_tree->Branch("m_mc_sum_neutral_m", &m_mc_sum_neutral_m,  "m_mc_sum_neutral_m/F");
//  .......
    ...
    m_tree->Branch("mc_higgs_m", &m_mc_higgs_m, "mc_higgs_m/F");
    m_tree->Branch("mc_higgs_e", &m_mc_higgs_e, "mc_higgs_e/F");
    m_tree->Branch("mc_higgs_rec_m", &m_mc_higgs_rec_m, "mc_higgs_rec_m/F");
    ...
    m_tree->Branch("mc_zw1_m", &m_mc_zw1_m, "mc_zw1_m/F");
    m_tree->Branch("mc_zw1_p", &m_mc_zw1_p, "mc_zw1_p/F");
    m_tree->Branch("mc_zw1_pt", &m_mc_zw1_pt, "mc_zw1_pt/F");
    m_tree->Branch("mc_zw1_e", &m_mc_zw1_e, "mc_zw1_e/F");
    m_tree->Branch("mc_zw1_rec_m", &m_mc_zw1_rec_m, "mc_zw1_rec_m/F");
    
    m_tree->Branch("mc_zw2_m", &m_mc_zw2_m, "mc_zw2_m/F");
    m_tree->Branch("mc_zw2_p", &m_mc_zw2_p, "mc_zw2_p/F");
    m_tree->Branch("mc_zw2_pt", &m_mc_zw2_pt, "mc_zw2_pt/F");
    m_tree->Branch("mc_zw2_e", &m_mc_zw2_e, "mc_zw2_e/F");
    m_tree->Branch("mc_zw2_rec_m", &m_mc_zw2_rec_m, "mc_zw2_rec_m/F");
    
    m_tree->Branch("mc_zw1zw2_m", &m_mc_zw1zw2_m, "mc_zw1zw2_m/F");
    m_tree->Branch("mc_zw1zw2_e", &m_mc_zw1zw2_e, "mc_zw1zw2_e/F");
    m_tree->Branch("mc_zw1zw2_rec_m", &m_mc_zw1zw2_rec_m, "mc_zw1zw2_rec_m/F");
...
void hig2inv::variable_init() {
    ...
    TLorentzVector beamp(0.0, 0.0, 120.0, 120.0);
    TLorentzVector beamm(0.0,0.0,-120.0,120.0);
    ...
    for(int i=0; i<4; i++) {
        ...
        P4_sum_p_501[i]= 0.0;
        P4_sum_p_21120[i]= 0.0;
        m_p_visible[i] = 0.0;
    //    MC information
        m_mc_p_dilepton[i]=0.0;
        m_mc_p_redilepton[i]=0.0;

    //merge three background data analysis
    P4_MC_neutral_sum[i]=0.0;
    P4_MC_photon_sum[i]=0.0;
    P4_MC_muon_sum[i]=0.0;
    P4_MC_electron_sum[i]=0.0;
    P4_MC_charge_sum[i]=0.0;
    ...
    FourMom_IsoMuonP.clear();
    FourMom_IsoMuonM.clear();
    FourMom_IsoEletronP.clear();
    FourMom_IsoEletronM.clear();
    P4_IsMuon.clear();
    P4_IsElectron.clear();
  ...
  // MC info.
    ...
    _TauTauM=0.0;
    _qqM=0.0;
    m_mc_init_n_lepton_plus = 0;
    m_mc_init_n_lepton_minus = 0;
    m_mc_lepton_minus_id = 0;
    m_mc_lepton_plus_id = 0;
    mc_n_charge = 0;
    m_mc_higgs_e = 0.0;
    m_mc_higgs_m = 0.0;
    m_mc_pdgid.clear();
    ...
    m_sum_e_charged=0.0;
    m_sum_muon_e=0.0;
    m_sum_electron_e=0.0;

    P4_MCTruth_qq_electron.clear();
    P4_MCTruth_qq_muon.clear();
    P4_MCTruth_qq_photon.clear();
    P4_MCTruth_qq_neutral.clear();
    P4_MCTruth_qq_charge.clear();

    m_mc_nparents.clear();
    m_mc_ndaughters.clear();
    m_mc_charge.clear();

    m_mc_init_qq_electron_e.clear();
    m_mc_init_qq_muon_e.clear();
    m_mc_init_qq_photon_e.clear();
    m_mc_init_qq_neutral_e.clear();
    m_mc_init_qq_charge_e.clear();
    ...
    m_mc_sum_muon_e=0.0;
    m_mc_sum_muon_m=0.0;
    m_mc_sum_electron_e=0.0;
    m_mc_sum_electron_m=0.0;

    m_mc_sum_photon_e=0.0;
    m_mc_sum_photon_m=0.0;

    m_mc_sum_charge_e=0.0;
    m_mc_sum_charge_m=0.0;
    m_mc_sum_neutral_e=0.0;
    m_mc_sum_neutral_m=0.0;
...
//changed:we need Neutral sum monmentum and energy,this is visible energy.
    ...
    for(int i = 0; i < nReco; i++) {
        ...
        if(a_Reco->getClusters().size()>0)
        {        
            Cluster *tmp_clu=a_Reco->getClusters()[0];
            m_total_clu_energy+=tmp_clu->getEnergy();
        }
        if(a_Reco->getCharge()!=0) continue;
//        if(fabs(a_Reco->getMomentum()[2]/a_Reco->getEnergy())>0.995) continue;
        ...
        m_Neutral_PID.push_back(a_Reco->getType());
        counter++;
    ...
    m_sum_e_neutral = P4_Neutral_Sum[3];
}
...
//        if(fabs(a_Reco->getMomentum()[2]/a_Reco->getEnergy())>0.995) continue; // acceptance of the detector
    ...
    m_sum_e_photon=P4_sum_p_photon[3];
    m_sum_pt_photon = sqrt(m_sum_p_photon[0]*m_sum_p_photon[0]+m_sum_p_photon[1]*m_sum_p_photon[1]);

    for(int i = 0; i < nReco; i++) {
        ReconstructedParticle *a_Reco = dynamic_cast<EVENT::ReconstructedParticle *>(col_Reco->getElementAt(i));
        if(a_Reco->getCharge()!=0) continue;
        if(a_Reco->getType()!= 501) continue;
//        if(fabs(a_Reco->getMomentum()[2]/a_Reco->getEnergy())>0.995) continue; // acceptance of the detector
            P4_sum_p_501[3] += a_Reco->getEnergy();
            P4_sum_p_501[0] += a_Reco->getMomentum()[0];
            P4_sum_p_501[1] += a_Reco->getMomentum()[1];
            P4_sum_p_501[2] += a_Reco->getMomentum()[2];
            
    }
    m_sum_e_501=P4_sum_p_501[3];
    for(int i = 0; i < nReco; i++) {
        ReconstructedParticle *a_Reco = dynamic_cast<EVENT::ReconstructedParticle *>(col_Reco->getElementAt(i));
        if(a_Reco->getCharge()!=0) continue;
        if(a_Reco->getType()!= 21120) continue;
//        if(fabs(a_Reco->getMomentum()[2]/a_Reco->getEnergy())>0.995) continue; // acceptance of the detector
            P4_sum_p_21120[3] += a_Reco->getEnergy();
            P4_sum_p_21120[0] += a_Reco->getMomentum()[0];
            P4_sum_p_21120[1] += a_Reco->getMomentum()[1];
            P4_sum_p_21120[2] += a_Reco->getMomentum()[2];
            
    }
    m_sum_e_21120=P4_sum_p_21120[3];
}
...
void hig2inv::selectCharged( int nReco, LCCollection* col_Reco ) {
    ...
    for(int i = 0; i < nReco; i++) {
        ...
        m_e_charged.push_back(a_Reco->getEnergy());
        m_charged_PID.push_back(a_Reco->getType());
        counter++;
    }
...
//    if ( RecoE > 0.0 && RecoE < RecoEMax) {
        ...
        if(RecoPID == MuonPID) {
            if 	(lepton_charge >  0.1){
                const EVENT::TrackVec &trkvec = a_Reco->getTracks();
                if (trkvec.size() != 0) n_muon_Ptrack++;
            }
            if (lepton_charge > 0.1)
            {
                const EVENT::TrackVec &trkvec = a_Reco->getTracks();
                if (trkvec.size() != 0) n_muon_Mtrack++;
            }
...
void hig2inv::selectIsoLepton(int nLepton,LCCollection* Col_Leps,std::vector<TLorentzVector> P4_jet){
    for(int i = 0; i < nLepton; i++) {
        ReconstructedParticle * a_Reco = dynamic_cast<ReconstructedParticle*>(Col_Leps->getElementAt(i));      
        if(a_Reco->getCharge()==0) continue;
        lepton_charge = a_Reco->getCharge();
        RecoPID = abs(getPID(a_Reco));
        RecoE = a_Reco->getEnergy();
...
//    if ( RecoE > 0.0 && RecoE < RecoEMax) {
        if(RecoPID == MuonPID) {
            if( lepton_charge >  0.1 ) FourMom_IsoMuonP.push_back( curr );
            if( lepton_charge < -0.1 ) FourMom_IsoMuonM.push_back( curr );	
            P4_IsMuon.push_back( curr );
        }    
        if(RecoPID == ElectronPID) {
            if( lepton_charge >  0.1 ) FourMom_IsoEletronP.push_back( curr );
            if( lepton_charge < -0.1 ) FourMom_IsoEletronM.push_back( curr );
            P4_IsElectron.push_back( curr );			   
        }
    
    }          
    m_n_IsoMuonP = FourMom_IsoMuonP.size();
    m_n_IsoMuonM = FourMom_IsoMuonM.size();
    m_n_IsoMuon = m_n_IsoMuonP + m_n_IsoMuonM;
    m_n_IsoEletronP = FourMom_IsoEletronP.size();
    m_n_IsoEletronM = FourMom_IsoEletronM.size();
    m_n_IsoEletron=m_n_IsoEletronP+m_n_IsoEletronM;

    saveDiMuon( FourMom_IsoMuonP, FourMom_IsoMuonM);
    saveDiElectron(FourMom_IsoEletronP, FourMom_IsoEletronM);
}
void hig2inv::saveDiMuon(std::vector<TLorentzVector> FourMom_IsoMuonP,std::vector<TLorentzVector> FourMom_IsoMuonM) {

    int nlep_p =  FourMom_IsoMuonP.size();
    int nlep_m =  FourMom_IsoMuonM.size();
    if (nlep_p > 0 && nlep_m >0){
        for(int i = 0; i < nlep_p; i++) {  
            for(int j = 0; j < nlep_m; j++) {
                P_P = FourMom_IsoMuonP[i];
                P_M = FourMom_IsoMuonM[j];	
                float currdimumass = (P_P + P_M).M();
                //This file don't exclude background,only record information
                if(fabs(currdimumass - ZMass) > MinZThrDis) continue;   
                MinZThrDis = fabs(currdimumass - ZMass);
                m_m_Isdimu = currdimumass;
                m_e_Isdimu = (P_P + P_M).E();
                m_p_Isdimu = (P_P + P_M).P();
    // save recil mass
                m_e_Isdimurec = (P_T - P_P - P_M).E();
                m_p_Isdimurec = (P_T - P_P - P_M).P();
                m_m_Isdimurec = (P_T - P_P - P_M).M();
            }
        }
    }
}
void hig2inv::saveDiElectron(std::vector<TLorentzVector> FourMom_IsoEletronP,std::vector<TLorentzVector> FourMom_IsoEletronM) {

    int nlep_p =  FourMom_IsoEletronP.size();
    int nlep_m =  FourMom_IsoEletronM.size();
    if (nlep_p > 0 && nlep_m >0){
        for(int i = 0; i < nlep_p; i++) {  
            for(int j = 0; j < nlep_m; j++) {
                P_P = FourMom_IsoEletronP[i];
                P_M = FourMom_IsoEletronM[j];	
                float currdimumass = (P_P + P_M).M();
                //This file don't exclude background,only record information
                if(fabs(currdimumass - ZMass) > MinZThrDis) continue;   
                MinZThrDis = fabs(currdimumass - ZMass);
                m_m_Isdie = currdimumass;
                m_e_Isdie = (P_P + P_M).E();
                m_p_Isdie = (P_P + P_M).P();
    // save recil mass
                m_e_Isdierec = (P_T - P_P - P_M).E();
                m_p_Isdierec = (P_T - P_P - P_M).P();
                m_m_Isdierec = (P_T - P_P - P_M).M();
            }
        }
    }
}
//get precise PID information 
    ...
    if (n_leptonp>0 && n_leptonm>0){
        ...
        for (int j=0; j< n_leptonm; j++){
            ...
            if (E_leptonp <min_value || E_leptonm<min_value){
                ...
                min_value=E_leptonp;
                TLorentzVector FourMinLeton1=FourMom_LeptonP[i];
                for (int k=0; k<4;k++){
                FourMin_Lepton[k]=FourMinLeton1[k];
                }
                }
                if (E_leptonp >= E_leptonm){
                min_value=E_leptonm;
                TLorentzVector FourMinLeton2=FourMom_LeptonM[j];
                for (int k=0; k<4;k++){                
                FourMin_Lepton[k]=FourMinLeton2[k];
                }
            }
            }
            if (E_leptonp >max_value || E_leptonm>max_value){
                if(E_leptonp>E_leptonm){
                max_value=E_leptonp;
                TLorentzVector FourMaxLeton1=FourMom_LeptonP[i];
                for(int k=0;k<4;k++){
                FourMax_Lepton[k]=FourMaxLeton1[k];
                }
                }
            
                if (E_leptonp <= E_leptonm){
                max_value=E_leptonm;
                TLorentzVector FourMaxLeton2=FourMom_LeptonM[j];
                for(int k=0;k<4;k++){
                FourMax_Lepton[k] = FourMaxLeton2[k];
                }
            }
            }            
            }
...
void hig2inv::saveVisible(TLorentzVector P4_Neutral_Sum,TLorentzVector P4_Charged_Sum) {
   ...
   m_visible_p = (P4_Charged_Sum +  P4_Neutral_Sum).P();
   m_visible_pt= (P4_Charged_Sum +  P4_Neutral_Sum).Pt();
...
void hig2inv::checkGenMCInfo(LCCollection *col_MC)
{
    ...
    for (int i = 0; i < ncol; i++)
    {
        ...
        int mother_pid = -9999;
        if (nparents > 0)
            mother_pid = pvec[0]->getPDG();

        // Save Initial Partons PID
        if (nparents == 0)
            h_mc_init_plist->Fill(pdgid);

        // Save Higgs Daughter PID
        if (nparents == 1 && mother_pid == HiggsPID)
            h_mc_higgs_dlist->Fill(pdgid);
...
//        }
        ...
        {
            LeptonID = 13;
            //Exclude the signal in ZH
            if (m_mc_higgs_decay_type == 1 && m_mc_zz_flag == 22)
            {
                if (nparents == 0)
                {
                    if (pdgid == MuonPID)
...
bool hig2inv::buildZToff() {
    ...
    if (Processname =="BKGM"||Processname =="BKGE"||Processname =="BKGQ"){
        if(Nmup == 1 && Nmum == 1) return false;
        else return true;
    }
    else {
        if (Nmup == 1 && Nmum == 1 ){
...
void hig2inv::printMCinfo( int MCParticleID, int flag, MCParticle *mcp, std::vector<MCParticle*> pvec, std::vector<MCParticle*> dvec ) {
    ...
    int MCstatus      =  mcp->getGeneratorStatus();
    int pdgid       =  mcp->getPDG();
    int nparents    =  pvec.size();
    ...
    std::cout << ", status=" << MCstatus << " : nParents = " << nparents << " : " ;
...
void hig2inv::saveMCTruthInfo(LCCollection *col_MC)
{
    ...
    for (int i = 0; i < ncol; i++)
    {
        ...
        //MCstatus = mcp->getGeneratorStatus();
        int pdgid = mcp->getPDG();
    ...
    if (fabs(pdgid)==MuonPID){
        float theta = fabs((p4vec - P_Z).Theta());
        if (theta < count){
            count = theta;
            m_muz_theta = theta * 180.0 / M_PI;
    ...
    // The final particles.   qq decay particles
    ...
    if(nparents != 0&&abs(pdgid) == ElectronPID&&ndaughters==1) {
            m_mc_init_qq_electron_e.push_back(e);
            P4_MCTruth_qq_electron.push_back(p4vec);
      }
    if(nparents != 0&&abs(pdgid) == MuonPID&&ndaughters==1) {
            m_mc_init_qq_muon_e.push_back(e);
            P4_MCTruth_qq_muon.push_back(p4vec);
      }
    if( nparents != 0&&pdgid==22&&ndaughters==1) {
            m_mc_init_qq_photon_e.push_back(e);
            P4_MCTruth_qq_photon.push_back(p4vec);
            m_mc_nparents.push_back(nparents);
            m_mc_pdgid.push_back(mother_pid);
      } 
    if( (nparents != 0&&charge==0&&abs(pdgid)!=12&&abs(pdgid)!=14&&abs(pdgid)!=16&&ndaughters==1)||(nparents != 0&&pdgid==22&&ndaughters==1)) {
            m_mc_init_qq_neutral_e.push_back(e);
            P4_MCTruth_qq_neutral.push_back(p4vec);
 
      }    

    if( nparents != 0&&abs(charge)>0.1&&ndaughters==1&&abs(pdgid)!=25) {
            m_mc_init_qq_charge_e.push_back(e);
            P4_MCTruth_qq_charge.push_back(p4vec);
            m_mc_nparents.push_back(nparents);
            m_mc_pdgid.push_back(mother_pid);
      }    
//    m_mc_pdgid.push_back(pdgid);
    
    m_mc_ndaughters.push_back(ndaughters);
    m_mc_charge.push_back(charge);
  }
  n_muon_track = Number;
  m_mc_n_Zboson = Zcount;
  m_mc_higgs_decay_type = DecayFlag;
  mc_n_charge = NCharge;
  mc_n_qq_neutral=P4_MCTruth_qq_neutral.size();
  mc_n_qq_photon=P4_MCTruth_qq_photon.size();  
  mc_n_qq_muon=P4_MCTruth_qq_muon.size();  
  mc_n_qq_electron=P4_MCTruth_qq_electron.size();  
  mc_n_qq_charge=P4_MCTruth_qq_charge.size();  
  fillMCInfo();
  fillMCqqInfo();
...
void hig2inv::fillMCInfo() {
  ...
  if( n_init_lepton_p > 0 && n_init_lepton_m > 0 ) {
    ...
    float pxll     = ( P4_MCTruth_LeptonPlus[0] + P4_MCTruth_LeptonMinus[0] ).Px();
    float pyll     = ( P4_MCTruth_LeptonPlus[0] + P4_MCTruth_LeptonMinus[0] ).Py();	
    ...
    float rec_pxll = (P_T - P4_MCTruth_LeptonPlus[0] - P4_MCTruth_LeptonMinus[0] ).Px();
    float rec_pyll = (P_T - P4_MCTruth_LeptonPlus[0] - P4_MCTruth_LeptonMinus[0] ).Py();
    float rec_pzll = (P_T - P4_MCTruth_LeptonPlus[0] - P4_MCTruth_LeptonMinus[0] ).Pz();
    float rec_ell = (P_T - P4_MCTruth_LeptonPlus[0] - P4_MCTruth_LeptonMinus[0] ).E();
    ...
    m_mc_p_dilepton[0]=pxll;
    m_mc_p_dilepton[1]=pyll;
    m_mc_p_dilepton[2]=pzll;
    m_mc_p_dilepton[3]=ell;
    m_mc_p_redilepton[0]= rec_pxll;
    m_mc_p_redilepton[1]= rec_pyll; 
    m_mc_p_redilepton[2]= rec_pzll;
    m_mc_p_redilepton[3]= rec_ell;
    ...
    m_mc_p_dilepton[0]=0.0;
    m_mc_p_dilepton[1]=0.0;
    m_mc_p_dilepton[2]=0.0;
    m_mc_p_dilepton[3]=0.0;
    m_mc_p_redilepton[0]= 0.0;
    m_mc_p_redilepton[1]= 0.0;
    m_mc_p_redilepton[2]= 0.0;
    m_mc_p_redilepton[3]= 0.0;
  }
...
void hig2inv::fillMCqqInfo() {

  int n_qq_neutral=P4_MCTruth_qq_neutral.size();
  int n_qq_photon=P4_MCTruth_qq_photon.size();  
  int n_qq_muon=P4_MCTruth_qq_muon.size();  
  int n_qq_electron=P4_MCTruth_qq_electron.size(); 
  int n_qq_charge=P4_MCTruth_qq_charge.size();   
  float max_value=0.0;
   for (int i=0; i < n_qq_neutral;i++){
        TLorentzVector P4_neutral = P4_MCTruth_qq_neutral[i];
        P4_MC_neutral_sum[0] += P4_neutral[0];
        P4_MC_neutral_sum[1] += P4_neutral[1];
        P4_MC_neutral_sum[2] += P4_neutral[2];
        P4_MC_neutral_sum[3] += P4_neutral[3];
   }

   for (int i=0; i < n_qq_photon;i++){
        TLorentzVector P4_photon = P4_MCTruth_qq_photon[i];
        P4_MC_photon_sum[0] += P4_photon[0];
        P4_MC_photon_sum[1] += P4_photon[1];
        P4_MC_photon_sum[2] += P4_photon[2];
        P4_MC_photon_sum[3] += P4_photon[3];
   }

   for (int i=0; i < n_qq_muon;i++){
        TLorentzVector P4_muon = P4_MCTruth_qq_muon[i];
        P4_MC_muon_sum[0] += P4_muon[0];
        P4_MC_muon_sum[1] += P4_muon[1];
        P4_MC_muon_sum[2] += P4_muon[2];
        P4_MC_muon_sum[3] += P4_muon[3];
   }

    for (int i=0; i < n_qq_electron;i++){
        TLorentzVector P4_electron = P4_MCTruth_qq_electron[i];
        P4_MC_electron_sum[0] += P4_electron[0];
        P4_MC_electron_sum[1] += P4_electron[1];
        P4_MC_electron_sum[2] += P4_electron[2];
        P4_MC_electron_sum[3] += P4_electron[3];
        if  (P4_electron[3] > max_value){
            max_value=P4_electron[3];
            m_mc_maxpe_electron=P4_electron[3];
        }

   }
    for (int i=0; i < n_qq_charge;i++){
        TLorentzVector P4_charge = P4_MCTruth_qq_charge[i];
        P4_MC_charge_sum[0] += P4_charge[0];
        P4_MC_charge_sum[1] += P4_charge[1];
        P4_MC_charge_sum[2] += P4_charge[2];
        P4_MC_charge_sum[3] += P4_charge[3];
   }

   m_mc_m_visible=(P4_MC_charge_sum+P4_MC_neutral_sum).M();
   m_mc_energy_visible=(P4_MC_charge_sum+P4_MC_neutral_sum).E();
   m_mc_visible_pt=(P4_MC_charge_sum+P4_MC_neutral_sum).Pt();
   m_mc_visible_p=(P4_MC_charge_sum+P4_MC_neutral_sum).P();
   TLorentzVector ecms(0,0,0,240);
   TLorentzVector mc_miss=ecms-P4_MC_charge_sum-P4_MC_neutral_sum;
   m_mc_miss_e=mc_miss.E();
   m_mc_miss_m=mc_miss.M();
   m_mc_sum_muon_e=P4_MC_muon_sum.E();
   m_mc_sum_muon_m=P4_MC_muon_sum.M();
   m_mc_sum_electron_e=P4_MC_electron_sum.E();
   m_mc_sum_electron_m=P4_MC_electron_sum.M();
   m_mc_sum_charge_e=P4_MC_charge_sum.E();
   m_mc_sum_charge_m=P4_MC_charge_sum.M();
   m_mc_sum_neutral_e=P4_MC_neutral_sum.E();
   m_mc_sum_neutral_m=P4_MC_neutral_sum.M();

   m_mc_sum_photon_e=P4_MC_photon_sum.E();
   m_mc_sum_photon_m=P4_MC_photon_sum.M();
}
...
int hig2inv::getPID(ReconstructedParticle* a_Reco){
    mvaVal_mu=0.0;
    mvaVal_e=0.0;
    mvaVal_pi=0.0;

    int pid;
    if(a_Reco->getParticleIDs().size()<3)pid = fabs(a_Reco->getType());
    //else if(a_RecoP->getEnergy()<5)RecoPID = a_RecoP->getType();
    else{
//			pid = a_RecoP->getType();
        for(int p=0;p<3;p++){
            if(abs(a_Reco->getParticleIDs()[p]->getPDG())==13){
                mvaVal_mu=a_Reco->getParticleIDs()[p]->getLikelihood();
            }
            else if(abs(a_Reco->getParticleIDs()[p]->getPDG())==11){
                mvaVal_e=a_Reco->getParticleIDs()[p]->getLikelihood();
            }
            else if(abs(a_Reco->getParticleIDs()[p]->getPDG())==211){
                mvaVal_pi=a_Reco->getParticleIDs()[p]->getLikelihood();
            }
        }

        if(a_Reco->getEnergy()<2){
            if(mvaVal_e>0.45&&mvaVal_mu<0.05&&mvaVal_pi<0.55){
                ClusterID = -11;
            }
            if(mvaVal_e<0.45&&mvaVal_mu>0.05&&mvaVal_pi<0.55){
                ClusterID = -13;
        ...
        else if(a_Reco->getEnergy()<5){
            if(mvaVal_e>0.4&&mvaVal_mu<0.05&&mvaVal_pi<0.6){
                ClusterID = -11;
            }
            if(mvaVal_e<0.4&&mvaVal_mu>0.05&&mvaVal_pi<0.6){
                ClusterID = -13;
        ...
        else if(a_Reco->getEnergy()<10){
            if(mvaVal_e>0.05&&mvaVal_mu<0.05&&mvaVal_pi<0.9){
                    ClusterID = -11;
            }
            if(mvaVal_e<0.05&&mvaVal_mu>0.05&&mvaVal_pi<0.9){
                    ClusterID = -13;
            ...
            if(mvaVal_e>0.05&&mvaVal_mu<0.02&&mvaVal_pi<0.96){
                    ClusterID = -11;
            }
            if(mvaVal_e<0.05&&mvaVal_mu>0.02&&mvaVal_pi<0.96){
                    ClusterID = -13;
...
bool hig2inv::preselection(){
    if(Processname=="muon"||Processname=="BKGM"){
        if(m_n_leptonp==1&&m_n_leptonm==1){
            h_evtflw->Fill(1);
            if (m_m_recoil > 120 && m_m_recoil < 150)
            {
                h_evtflw->Fill(2);
                return true;
            }
    ...
    if (Processname == "ee" || Processname == "BKGE")
    {
        if (m_n_leptonp == 1 && m_n_leptonm == 1)
        {
            h_evtflw->Fill(1);
            if (m_m_recoil > 120 && m_m_recoil < 170)
            {
                h_evtflw->Fill(2);
                return true;
            }
